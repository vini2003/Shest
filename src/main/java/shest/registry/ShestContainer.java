...
public class ShestContainer {
    ...
    public static void initialize() {
        ContainerProviderRegistry.INSTANCE.registerFactory(SHEST_BLOCK_CONTAINER,
                (syncId, id, player, buffer) -> new shest.common.ShestContainer(syncId, buffer.readText(), player.inventory, buffer.readBlockPos(), buffer.readInt(), buffer.readInt(), buffer.readInt()));
    }
}